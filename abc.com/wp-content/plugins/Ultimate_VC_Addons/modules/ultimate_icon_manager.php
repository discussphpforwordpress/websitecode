...
if ( ! class_exists( 'AIO_Icon_Manager' ) ) {
	class AIO_Icon_Manager {
		...
		public function get_font_manager( $id ) {
			...
			$fonts  = get_option( 'smile_fonts' );
			$fonts  = get_option( 'smile_fonts' );
			$output = '<p><div class="preview-icon preview-icon-' . esc_attr($id) . '"><i class=""></i></div><input class="search-icon" type="text" placeholder="搜索合适的图标." /></p>';
			...
			foreach ( $fonts as $font => $info ) {
				...
				if ( ! empty( $icon_set ) ) {
					...
					if ( $font == 'smt' || $font == 'Defaults' ) {
						$output .= '<h3 class="icon_font_name"><strong>' . __( "默认图标", "smile" ) . '</strong>';
					} else {
					...
					if ( $n != 1 ) {
						$output .= '<button class="button button-secondary button-small smile_del_icon" data-delete=' . esc_attr($font) . ' data-title="Delete This Icon Set">删除图标集</button>';
					}
					$output .= '</h3>';
