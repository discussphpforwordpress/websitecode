...
function presscore_register_scripts()
{
	...
	wplog('$template_uri:'.$template_uri);
	...
	wp_register_script('dt-upload', "{$template_uri}/uploader/js/upload.js", array('jquery'), '', true);
}
	...
	function presscore_body_class($classes)
{
	...
	if (presscore_header_with_bg() && !presscore_header_layout_is_side()) {
		...
		if ($config->get_bool('header.slideshow.header_below')
