		...
		<?php presscore_get_template_part( 'theme', 'header/mixed-branding' );
			...
			if ( 'center' == $config->get( 'header.mixed.view.top_line.logo.position' ) || 'left' == $config->get( 'header.mixed.view.top_line.logo.position' ) ) {
				echo '<div class="top-line-left ' . $top_line_left_classes .'" >';
					echo  presscore_render_header_elements( 'side_top_line', 'left-widgets' );
				echo '</div><div class="top-line-right ' . $top_line_right_classes .'">';
					echo  presscore_render_header_elements( 'top_line_right', 'right-widgets' );

					echo  presscore_header_menu_icon();
				echo '</div>';
