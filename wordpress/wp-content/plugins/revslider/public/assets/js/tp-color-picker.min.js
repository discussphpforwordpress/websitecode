...
window.RevColor = {
	...
	processRgba: function (b,
		...
		var l = "undefined" !== typeof k,
			p = (l ? "rgba" : "rgb") + "(" + parseInt(b.substring(0, 2), 16) + ", " + parseInt(b.substring(2, 4), 16) + ", " + parseInt(b.substring(4, 6), 16);
...
(function (b) {
	...
	function y() {
		...
		if (a === ia || c) return a = b(this), a.closest(".rev-cpicker-presets-group").find(".rev-cpicker-color.selected").removeClass("selected"), q = a, ha && !xa && ya(q.data("gradient"), !0), q.addClass("selected"), !1
	}
	...
	function bb(a, c) {
		...
		e = d.color;d = $a(e, a, !0);e = Za(e, !0);d = Ya(a, c, e,
			d);f && f.removeClass("active");f = b(d).addClass("active").appendTo(L).draggable(V);la = f.children(".rev-cpicker-point-square")[0];ma = f.children(".rev-cpicker-point-triangle")[0];Q = L.children();e = cb(c);x(d);
	...
	function cb(a) {
		...
		if (d) E.hasClass("active") && (W.attr("disabled", "disabled"), M.attr("disabled", "disabled"), E.removeClass("active")), b = RevColor.rgbaToHex(b)[0], X.css("background",
			b), N.removeAttr("disabled").val(a + "%"), 2 < n.find(".rev-cpicker-point-bottom").length && F.addClass("active"), n.addClass("open");
		else {
	...
	function eb() {
		...
		for (var a = v.length, b = a - 1, d, e, g, h, f = 0; f < a; f++)
			if (d = v[f], g = d.align, e = hb(f, g), !1 === e && (e = ib(f, g, b)), h = ib(f, g, b), !1 === h && (h = hb(f, g)), "bottom" === g) {
				g = d;
				var r = e.alpha,
					k = h.alpha;
	...
	function kb() {
		var a = b(this); - 1 !== this.className.search("down") ? (a.parent().addClass("active"), a.closest(".rev-cpicker-presets").addClass("active"), b(this.id.replace("-btn", "")).addClass("active"),
	...
	function Hb() {
				...
				if (d && isNaN(d)) {
					...
					if (!a) {
						...
						I.off("revcpicker_onajax_save.revcpicker").on("revcpicker_onajax_save.revcpicker", function (a, d) {
							if (d) n.removeClass("onajax onajaxsave"), alert(c.attr("data-message"));
							else {
		...
		b(".rev-cpicker-hex").on("focusout.revcpicker", function () {
			...
			if ("rev-cpicker-color-hex" === this.id) {
				if (a = t.val())
					if (a = RevColor.sanitizeHex(a), RevColor.isColor.test(a)) t.val(a);
	...
	function rb(a) {
		...
		for (var e = 0; e < d; e++) {
			...
			var f = g;
			var k = h.slice() || [];
			for (var n = document.createDocumentFragment(), m = -1 !== f.search("core"), q = m ? "" : " rev-picker-color-custom", l = k.length, p = -1 !== f.search("colors") ? 5 : 6, l = Math.max(Math.ceil(l / 6), p), p = 0; p < l; p++)
	...
	var I, z, qb, Y, ja, v, fb, ea, za, tb, fa, n, Na, J, H,
		ra, Ua, ta, ub, vb, X, D, t, N, R, Ga, f, oa, Pa, Ea, wb, xb, Ka, aa, ba, sa, Ca, la, ma, K, Va, gb, Ha, Qa, Oa, xa, ca, jb, pa, ka, P, na, M, ab, Sa, W, Q, L, nb, F, T, sb, yb, ia, m, S, ob, w, G, Wa, zb, A, Ta, ha, qa, pb, Z, E, q, Ma, La, mb, Ab, Ra, C = {},
		V = {
			axis: "x",
		...
		wa = {
			...
			solid_color: "纯色",
			gradient_color: "渐变颜色",
			currently_editing: "当前编辑",
			core_presets: "核心预设",
			custom_presets: "自定义预设",
			enter_a_name: "输入名字",
			save_a_new_preset: "保存新预设",
			save: "保存",
			color_hex_value: "颜色十六进制值",
			opacity: "透明度",
			clear: "清除",
			location: "位置",
			"delete": "删除",
			...
			radial: "径向",
			enter_angle: "输入角度",
			reverse_gradient: "反向渐变",
			delete_confirm: "删除此自定义预设颜色？",
			naming_error: "请输入新颜色预设的唯一名称"
		},
		ua = "#FFFFFF #000000 #FF3A2D #007AFF #4CD964 #FFCC00 #C7C7CC #8E8E93 #FFD3E0 #34AADC #E0F8D8 #FF9500".split(" "),
		va = [{
		...
		return a &&
			"string" === typeof a ? this.each(Ib[a]) : this.each(function () {
				...
				if (c.hasClass("rev-cpicker-component")) c.tpColorPicker("refresh");
					...
					var d = b('<span class="rev-colorpicker"></span>').data("revcolorinput", c),
					...
					c.attr({
						type: "hidden",
						"data-color": l
					}).data("tpcp", e).addClass("rev-cpicker-component");
					m && d.addClass(m);
					f ?
