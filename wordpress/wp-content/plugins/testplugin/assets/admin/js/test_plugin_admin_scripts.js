...
jQuery(document).ready(function($, undefined) {	
	...
	/ ---------------------------------------------------------------------- */	
		...
		var $wpContent = $('#wpcontent');
			$TestPluginAdmin = $('#test-plugin-admin-wrap'),
			$TestPluginForm = $TestPluginAdmin.find('#test-plugin-form');
		...
		$TestPluginAdmin.on('blur', 'input[type="text"][data-popup], textarea[data-popup]', function(e) {			
		...
		$TestPluginAdmin.on('click', '[data-action*="font-style"]', function(e) {
			var $this = $(this);
		...
		$TestPluginAdmin.on('click', '[data-action="add-tag"]', function(e) {
			var $this = $(this), 
		...
		$TestPluginAdmin.on('click', '.gwa-tag-remove', function(e) {
			var $this = $(this);
		...
		$TestPluginAdmin.on('click', '.gwa-col-box-header', function() {
		...
		$TestPluginAdmin.on('focusin focusout', '.gwa-textarea-btn textarea', function(e) {
			var $this = $(this);
			if (e.type == 'focusin' ) {
		...
		/* Open & Close Admin Box panels */
		$TestPluginAdmin.on('click', '.gwa-abox-tab', function(e) {
		...
		/* Open & Close Admin Box panels */
		$TestPluginAdmin.on('click', '.gwa-abox-header-tab', function(e) {
			var $this=$(this), 
		...
		/* Open & Close Admin Box panels */
		$TestPluginAdmin.on('click', '.gwa-abox-header', function(e) {
		...
		/* Select Help Mode */
		$TestPluginAdmin.on('change', 'select[data-action="help"]', function() {
			$TestPluginAdmin.find('.gwa-pcontent, .gwa-popup').attr('data-help', $(this).val());
			document.cookie='test_plugin[settings][help]['+userSettings.uid+']='+$(this).val();
		});
		
		var setHelpCookie = function() {
			if (document.cookie.indexOf('test_plugin[settings][help]['+userSettings.uid+']') == -1) {
				document.cookie='test_plugin[settings][help]['+userSettings.uid+']=1';
				$TestPluginAdmin.find('.gwa-pcontent').attr('data-help', 1);
			}
		}
		setHelpCookie();
		
		/* Select Help Mode */
		$TestPluginAdmin.on('change', 'select[data-action="tm-rows"]', function() {
			var $this =$(this), $parent = $TestPluginAdmin.find('#test-plugin-table-manager');
			document.cookie='test_plugin[settings][tm-rows]['+userSettings.uid+']='+$(this).val();
			if ($this.val()==5) {
		...
		/* Set Top Bar */
		function setTopBar() {
			var $topBar = $TestPluginAdmin.find('.gwa-ptopbar');
		...
		/* Site loader */
		function setLoader() {
			var $siteLoader = $TestPluginAdmin.find('.gwa-site-loader');
		...
		function showLoader(pageEffect, fullScreen) {
			...
			if (pageEffect===true) $TestPluginAdmin.addClass('gwa-loading');
			
			if ($TestPluginAdmin.find('#gwa-popup-wrap').length || fullScreen===true) {
		...
		function hideLoader() {
			...
			$TestPluginAdmin.removeClass('gwa-loading');
		...
		function showAjaxMsg(message, type) {			
			...
			$TestPluginAdmin.find('.gwa-ptopbar').append($newMsg);
		...
		$TestPluginAdmin.on('click', '.gwa-ajaxmsg-close', function(e) {
			e.preventDefault();
			clearTimeout(ajaxMsgTimeout);
			hideAjaxMsg();
		});		
		...
		function handleFileSelect($elem, file) {
			...
			if ( file.name ) {
				...
				if ( $input.data('allowed-ext') ) {
					...
					if ( fileExt.length == 1 || fileExt[fileExt.length-1].toLowerCase() != $input.data('allowed-ext') ) {
						showAjaxMsg('<p><strong>'+TestPluginL10n.upload_ext+'</strong></p>', 'error');
						return;
					}
				...
				if ( $input.data('max-size') ) {
					...
					if ( !file.size ) {
						showAjaxMsg('<p><strong>'+TestPluginL10n.upload_size_null+'</strong></p>', 'error');
						return;
					}
					if ( file.size > $input.data('max-size') ) {
						showAjaxMsg('<p><strong>'+TestPluginL10n.upload_size+'</strong></p>', 'error');
						return;						
					}
			...
			$form.find(':input').each(function(index, element) {
				...
				formdata.append('action' , 'test_plugin_ajax_action');	
				switch(element.type) {
			...
			$.ajaxPrefilter(function( options, originalOptions, jqXHR ) {
				...
				if (!formdata || $TestPluginAdmin.find('#test-plugin-form')[0].name != "impex-form" && $TestPluginAdmin.find('#test-plugin-form')[0].name != "plugin-update-form" || ( $TestPluginAdmin.find('#test-plugin-form')[0].name == "impex-form"  && $TestPluginAdmin.find('[name="_action_type"]').val() != 'import' ) ) {
		...
		$TestPluginAdmin.on('dragover dragleave drop', '.gwa-dnd-upload', function(e) {
		...
		$TestPluginAdmin.on('change', '.gwa-dnd-upload input[type="file"]', function(e) {
		...
		$TestPluginAdmin.on('click', '[data-action="dnd-upload"]', function(e) {
		...
		function seachTable($elem) {
			if (!$elem || $TestPluginAdmin.find('#test-plugin-table-manager').data('cnt') == 0) return;
			
			var $parent = $elem.closest('.gwa-search-input'), 
				$input = $parent.find('input[type="text"]'),
				$label = $parent.find('span'),
				$assets = $TestPluginAdmin.find('.gwa-search-assets');
				$filter = $TestPluginAdmin.find('[data-action="tm-filter-skin"]');
				thumbCnt = 0;

			$TestPluginAdmin.find('#test-plugin-table-manager').find('.gwa-thumb').not('.gwa-thumb-new').each(function(index, element) {		
			...
			$TestPluginAdmin.find('#test-plugin-table-manager').data('filtered-cnt', thumbCnt)
			
			if (thumbCnt == 0) {
				$TestPluginAdmin.find('#test-plugin-table-manager .gwa-dash-nores').show();
			} else {
				$TestPluginAdmin.find('#test-plugin-table-manager .gwa-dash-nores').hide();
			}
							
			loadThumbInview();
		};
		
		$TestPluginAdmin.on('keydown','[data-action="tm-search"] input[type="text"]', function(e) {
		...
		$TestPluginAdmin.on('click','[data-action="tm-search"] a', function(e) {
			seachTable($(this));
			e.preventDefault();
		});
		
		$TestPluginAdmin.on('change','[data-action="tm-filter-skin"]', function(e) {
			$TestPluginAdmin.find('[data-action="tm-search"] a').trigger('click');
		});
		
		$TestPluginAdmin.on('click','[data-action="tm-search-reset"]', function(e) {
			$TestPluginAdmin.find('[data-action="tm-search"] input[type="text"]').val('');
			$TestPluginAdmin.find('[data-action="tm-filter-skin"]').prop('selectedIndex',0);	
			$TestPluginAdmin.find('[data-action="tm-search"] a').trigger('click');					
			e.preventDefault();
		});
		
		/* Select thumbnail number of rows to show */
		if (typeof $TestPluginAdmin.data('settings') != undefined || !$TestPluginAdmin.data('settings').hasOwnProperty('tm-max-rows')) {
			$TestPluginAdmin.data('settings', $.extend( $TestPluginAdmin.data('settings'), { 'tm-max-rows' : 3 }));
			setTimeout(function() { 
				$TestPluginAdmin.find('[data-id="tm-max-rows"]').val($TestPluginAdmin.data('settings')['tm-max-rows']).trigger('change');
			}, 5);
		};

		$TestPluginAdmin.on('change','#test-plugin-table-manager [data-id="tm-max-rows"]', function() {
			var $this = $(this), $parent = $TestPluginAdmin.find('#test-plugin-table-manager');
			if ($this.val() == 5) {
		...
		function loadThumbInview() {
			var $parent = $TestPluginAdmin.find('#test-plugin-table-manager .gwa-thumbs'),
			...
			if ($parent.length) {	
				$TestPluginAdmin.find('.gwa-thumb').each(function(index, element) {
				...
				$('.gwa-thumbs-assets').css('right',$TestPluginAdmin.find('.gwa-thumbs').outerWidth()-$TestPluginAdmin.find('.gwa-thumbs')[0].scrollWidth);
		...
		$(window).on('scroll',function() { 
			...
			if ($TestPluginAdmin.find('#test-plugin-table-manager').length) {
		...
		$TestPluginAdmin.find('#test-plugin-table-manager .gwa-thumbs').on('scroll',function() {  loadThumbInview(); });
		$(window).on('resize', function() { loadThumbInview(); });			
		
		function getThumbIndex() {
			$TestPluginAdmin.find('#test-plugin-table-manager .gwa-thumb').each(function(index, element) {
		...
		function selectThumbs(i, deselect, inverse) {
			var $aBox= $TestPluginAdmin.find('#test-plugin-table-manager'),
				$thumbs = $TestPluginAdmin.find('#test-plugin-table-manager .gwa-thumb').not('.gwa-thumb-new'), 
			...
			$TestPluginAdmin.find('#test-plugin-tm-select').val(selected.join(',')).data('count', selected.length);
			showHideSc($TestPluginAdmin.find('#test-plugin-tm-select').data('count'));
		};
		
		function showHideSc(cnt) {
			
			var $headerInfo = $TestPluginAdmin.find('.gwa-abox-header-info'),
				$currentThumb = $TestPluginAdmin.find('.gwa-thumb.gwa-current');
				
				$TestPluginAdmin.find('.gwa-thumbs-assets .gwa-counter').text(cnt);
			
			if (cnt == 1) {
				$headerInfo.addClass('gwa-show');
				$headerInfo.find('input').val('[test_plugin id="'+$currentThumb.data('table-id')+'"]');
			} else {
		...
		/* Thumbnail hover */	
		$TestPluginAdmin.on('mouseenter mouseleave focusin focusout', '#test-plugin-table-manager .gwa-thumb', function(e) {
			var $this = $(this), 
			...
			if (e.type=='mouseenter' || e.type=='focusin') {
				if (e.type=='mouseenter') $('#test-plugin-table-manager .gwa-thumb').find('.gwa-assets-nav').removeClass('gwa-show');
		...
		$(document).delegate(this, 'keydown keyup', function(e) {
			...
			if (e.type == 'keydown') {
				...
				if(code == 46) { $TestPluginAdmin.find('.gwa-assets-nav.gwa-show a[data-action="delete"]').trigger('click'); }	
		...
		$TestPluginAdmin.on('dblclick click keydown', '.gwa-thumb', function(e) {
			var $this = $(this), 
				$parent = $this.closest('.gwa-thumbs'),
			...
			if (e.type == 'keydown') { 
				...
				$('#test-plugin-table-manager .gwa-thumb').find('.gwa-assets-nav').removeClass('gwa-show');
				$this.trigger('click');
		...
		$TestPluginAdmin.on('click', '.gwa-thumb-link', function(e) {
		...
		$TestPluginAdmin.on('click', this, function(e){
			if (!$(e.target).closest('.gwa-thumb').length && !$(e.target).closest('.gwa-thumbs-assets').length && !$(e.target).closest('.gwa-abox-header-info').length) {
				$TestPluginAdmin.find('.gwa-thumbs-assets').removeClass('gwa-show');	
				selectThumbs(-1);
			}
		});
				
		/* Show & hide thumb assets */
		$TestPluginAdmin.on('mouseenter mouseleave', '.gwa-assets-nav a, .gwa-col-assets-nav-main a', function(e) {
			var $this = $(this),
		...
		/* Show assets */
		$TestPluginAdmin.on('click', '.gwa-thumb .gwa-assets-nav a', function(e) {
			selectThumbs($(this).closest('.gwa-thumb').data('index'), false, true)
		});
		
		$TestPluginAdmin.on('click change', '[data-action]', function(e) {
			...
			if ($TestPluginAdmin.find('#test-plugin-table-manager').length) {			
				...
				if (typeof $this.data('action') !== 'undefined') {		
					if ($this.data('action')=='create') { 
						...
						$form.find('#test-plugin-tm-select').val('');
						$form.submit();
					}
		...
		var ajaxRq = null;
		$TestPluginAdmin.on('submit', '#test-plugin-form', function(e) {
			...
			/* Table editor page */
			if (formName  == "te-form") {
				$TestPluginAdmin.find('.test-plugin-col').each(function(index, element) {
				});
			}
			...
			ajaxRq = $.ajax({  
				type: 'post',
				url: ajaxurl, 
				data: $.param({ action: 'test_plugin_ajax_action' })+'&'+$this.serialize(),
				beforeSend: function () { }
			}).always(function() {
				hideLoader();
				$TestPluginAdmin.find('#result').remove();
			}).fail(function(jqXHR, textStatus) {
				showAjaxMsg('<p><strong>'+$TestPluginAdmin.data('ajaxerror')+'</strong></p>', 'error');
				
			}).done(function(data, textStatus, jqXHR) {
								
				if (data == 0) showAjaxMsg('<p><strong>'+TestPluginL10n.ajax_error+'</strong></p>', 'error');
				...
				if (formName == "tm-form") {
					if ( $ajaxResponse.find('.gwa-thumbs').length ) {
						$TestPluginAdmin.find('.gwa-thumbs').html( $ajaxResponse.find('.gwa-thumbs').html() );
						$TestPluginAdmin.find('.gwa-thumbs-assets').removeClass('gwa-show');
						$TestPluginAdmin.find('.gwa-thumb').addClass('gwa-notrans').css({ 'opacity' : 1, 'top': 0 });
						/* ?? */ $TestPluginAdmin.find('#test-plugin-table-manager .gwa-abox-content-header-cell-left').html($ajaxResponse.find('#test-plugin-table-manager .gwa-abox-content-header-cell-left').html());
						$TestPluginAdmin.find('#test-plugin-table-manager .gwa-abox-title span').html( $ajaxResponse.find('#test-plugin-table-manager .gwa-abox-title span').html());
						getThumbIndex();
						loadThumbInview();
					};
				}
				...
				if (formName == "te-form") {
					...
					if ($ajaxResponse.find('#postid').length && !$TestPluginAdmin.find('[name="postid"]').length) {	
						$this.prepend('<input type="hidden" name="postid" value="'+$ajaxResponse.find('#postid').text()+'">');
						$TestPluginAdmin.find('[data-popup="live-preview-edit"]').data('id', $ajaxResponse.find('#postid').text());
						$TestPluginAdmin.find('[data-popup="live-preview-edit"]').data('popup-subtitle', $TestPluginAdmin.find('[name="name"]').val());
						$TestPluginAdmin.find('[data-popup="live-preview-edit"]').data('alert', '');
						$TestPluginAdmin.find('.gwa-shortcode-row').show();
									
						if (window.history.replaceState) {
							history.replaceState({
								content: $('form').html()+'-',
								form: 'test-plugin-form'						
					...
					$TestPluginAdmin.find('.gwa-shortcode-row input').val('[test_plugin id="'+$TestPluginAdmin.find('[name="id"]').val()+'"]');
				...
				if (formName == "import-form" || formName == "impex-form") {
					...
					$TestPluginAdmin.find('select').trigger('change');
				};
		...
		function setColWidth(width) {
			...
			var $editor = $TestPluginAdmin.find('#test-plugin-column-editor'),
				$cols = $editor.find('.test-plugin-col'),
			...
			$cols.closest('.test-plugin-cols').css('width',colWidth+278);
		...
		function setColIndex() {
			...
			if ($cols.length) {
				$editor.find('.test-plugin-col-new').css('margin-top', 48);
			} else {
				$editor.find('.test-plugin-col-new').css('margin-top', 10);
			};
			...
			for (var x=0; x < $cols.length; x++) {
				...
				colWidth += $elem.outerWidth(true);				
				$elem.data('index', x).find('.test-plugin-col-index').html('#'+(x+1)).end().find('div.test-plugin-col-index').val(x);
		...
		function setSortableCols() {
			if (typeof jQuery().sortable !== 'undefined') {
				var $colsParent = $TestPluginAdmin.find('#test-plugin-column-editor').find('.test-plugin-cols');
				$colsParent.sortable({
					axis: 'x',
					revert: 100,
					items: '.test-plugin-col',
					distance: 5,
					handle: '.gwa-abox-header',
					scrollSpeed: 30,
					scrollSensitivity: 50,
					placeholder: 'test-plugin-col-placeholder',
					tolerance: 'pointer',
					start: function( event, ui ) { 
						ui.item.closest('.test-plugin-cols').css('height', ui.item.outerHeight(true))
						ui.item.siblings('.test-plugin-col').css('opacity',0.5)
					...
					update: function( event, ui ) { 
						...
						ui.item.closest('.test-plugin-cols').css('height', 'auto')
					}				
				});	
			};			
		};
		...
		$.setSytle = function($elem) {
			...
			var mainparent = $TestPluginAdmin.find('.test-plugin-cols');
			var html2=[];
			if ($elem === undefined) {
				$elem = $TestPluginAdmin.find('select[name*="[col-style-type]"]');
			}
			
			for (var x=0; x < $elem.length; x++) {
				var $el = $elem.eq(x),
					el = $elem[x],
					$parentCol = $el.closest('.test-plugin-col');
					index = el.selectedIndex,
					styleType = $el.find('option').eq(index).data('type');
				
				if (typeof styleType === 'undefined') return;
					
				$parentCol.find('.test-plugin-style-type').filter(function(index) {
		...
		$TestPluginAdmin.on('click', '.gwa-textarea-align a', function(e) {
		...
		$TestPluginAdmin.on('change', 'select[name*="[col-style-type]"]', function(e) {
			var foo ='[data-parent-id="'+this.name+'"][data-parent-value="'+this.value+'"] select'
			$(this).closest('.test-plugin-col').find(foo).trigger('change', true);
		});

		$TestPluginAdmin.on('change', 'select[name*="[col-style-type]"]', function(e, force) {
			var $this = $(this), $parentCol = $this.closest('.test-plugin-col'), family = $parentCol.find('select[name="col-style-family"]').val();
			if (($this).is(':visible') || force === true) $.setSytle($this);
		});
		
		
		$TestPluginAdmin.on('click', '.test-plugin-col', function(e) {
			if ($TestPluginAdmin.find('#test-plugin-column-editor').data('ready') === undefined) {
				$TestPluginAdmin.find('#test-plugin-column-editor').data('ready',true)
			} 
		});
		...
		$TestPluginAdmin.on('click', '[data-action="delete-col"], [data-action="clone-col"], [data-action="expand-col"]', function(e) {
			var $this = $(this), $parentCol = $this.closest('.test-plugin-col');
			if ($this.data('action')=="delete-col") {
				if (confirm($this.data('confirm'))){
					$this.closest('.test-plugin-col').remove();
					setColIndex();
					updateColCnt();
				};	
		...
		$TestPluginAdmin.on('change', '#test-plugin-style', function(e) {
		...
		function reloadEditor() {
				...
				$.ajax({  
					type: 'post',
					url: ajaxurl, 
					data: $.param({ action: 'test_plugin_ajax_action', _action :'editor_columns', _nonce : $TestPluginAdmin.find('form').find("#_nonce").val(), postid : ( $TestPluginAdmin.find('[name="postid"]').length ? $TestPluginAdmin.find('[name="postid"]').val() : null ), param : editorParams }),
					beforeSend: function () {
						showLoader(true);
					}
				}).always(function() {
					...
					if ( $resp.find('#test-plugin-global-style').length && !$resp.find('#test-plugin-global-style').is(':empty') ){
						$TestPluginAdmin.find('#test-plugin-global-style').html($resp.find('#test-plugin-global-style').html());
					} else {
						$TestPluginAdmin.find('#test-plugin-global-style').html('');
					}

					$TestPluginAdmin.find('.test-plugin-col').remove();
					$resp.find('.test-plugin-col').insertBefore($TestPluginAdmin.find('.test-plugin-col-new'));
					setColIndex();
					$.setSytle();					
		...
		$TestPluginAdmin.on('click', '[action="add-col"]', function(e) {

			$TestPluginAdmin.find('.test-plugin-col-new-link').trigger('click');
			e.preventDefault();
		...
		function updateColCnt() {
			...
			var colCnt = $TestPluginAdmin.find('.test-plugin-cols .test-plugin-col').length;
			$TestPluginAdmin.find('#test-plugin-column-editor .gwa-abox-title span').text('('+colCnt+')');
			
		};

		$TestPluginAdmin.on('mouseenter mouseleave click', '.test-plugin-col-new-link', function(e) {
			var $this = $(this).closest('.test-plugin-col-new'), 
				$cols = $this.closest('.test-plugin-cols').find('.test-plugin-col');
			...
			if (e.type=='mouseenter') {
				...
				if ($cols.length==10) {
					alert( TestPluginL10n.warning_maxcol );
					return;
				}
				...
				showLoader();
				$TestPluginAdmin.addClass('gwa-loading');
				$.ajax({  
					type: 'post',
					url: ajaxurl, 
					data: $.param({ 
						action: 'test_plugin_ajax_action', 
		...
		function setSortableRowsBtns() {
			if (typeof jQuery().sortable !== 'undefined') {
				var $colsParent = $TestPluginAdmin.find('#test-plugin-column-editor .test-plugin-cols');

				$TestPluginAdmin.find('#test-plugin-column-editor').find('.gwa-col-box-wrap').each(function(index, element) {
				$(element).sortable({
					axis: 'y',
					revert: 100,
					items: '.gwa-col-box',
		...
		$TestPluginAdmin.on('click', '.test-plugin-col', function() {
		...
		$TestPluginAdmin.on('click', '[data-action="delete-row"], [data-action="clone-row"], [data-action="add-row"], [data-action="delete-footer-row"], [data-action="clone-footer-row"], [data-action="add-footer-row"]', function(e) {
			...
			if ($this.data('action')=="delete-row" || $this.data('action')=="delete-footer-row") {
				...
				if ($('#test-plugin-column-editor').data('jump-row')) {
				...
				$.ajax({  
					type: 'post',
					url: ajaxurl, 
					data: $.param({ action: 'test_plugin_ajax_action', _action : currentAction, _nonce : $this.closest('form').find("#_nonce").val(), col_index : $this.closest('.test-plugin-col').data('index') }),
					beforeSend: function () {
						showLoader(true);
					}
				}).always(function() {
		...
		$TestPluginAdmin.on('change', 'select, .gwa-checkbox input[type=checkbox]', function(e) {
			showHideChilds($(this));
			if (!$popupTarget) DoCodeMirror();
		});

		/* new */		
		$TestPluginAdmin.on('click', '.gwa-img-upload-media-remove', function(e) {
			var $this = $(this), 
		...
		$TestPluginAdmin.on('change keydown', '.gwa-img-upload input[type="text"]', function(e) {
			var $this = $(this), 
				$parent = $this.closest('.gwa-img-upload');
			...
			if ((e.type == 'keydown' && code == 13) || e.type == 'change') {
				if ($this.val() != '') {
					...
					newImg.onerror=function(){ alert(TestPluginL10n.warning_invalid_imge); $media.find('a').trigger('click'); $this.val(''); }
					newImg.src=$(this).val();
				} else {
					$parent.find('.gwa-input-btn').show();
		...
		$TestPluginAdmin.on('change', '.gwa-img-upload select', function(e) {
			var $this = $(this), 
				$parent = $this.closest('.gwa-img-upload'),
				$input = $parent.find('input[type="text"]'),
				$input_alt = $parent.find('input[data-attr="alt"]'),
		...
		$TestPluginAdmin.on('click', '[data-action="img-upload"], [data-action="file-upload"]', function(e) {
			var $this = $(this),
		...
		/* Img selector */
		$TestPluginAdmin.on('change', '.gwa-img-selector', function(e) {
			var $this = $(this), 
			...
			if (typeof imgData !== 'undefined' && imgData != '') {
					...
					newImg.onerror=function(){ alert(TestPluginL10n.warning_invalid_imge); }
					newImg.src=imgData;				
			} else {
		...
		$(document).on('mousedown mouseup mousemove', function(e) {
			var $target = $(e.target),
				$colEditor = $('#test-plugin-column-editor').find('.test-plugin-cols-wrap'),
				$col = $target.closest('.test-plugin-col');
			...
			if (e.type=="mousemove") {
				...
				if  (clickPressed!=null) {
					$TestPluginAdmin.find('*:focus').blur();
					$colEditor.scrollLeft(clickPressed-e.pageX);
					e.preventDefault();
				} else {
					if (!$col.length && $colEditor.width()<$colEditor.find('.test-plugin-cols').outerWidth()) {
			...
			if ($colEditor.length && $colEditor.width()<$colEditor.find('.test-plugin-cols').outerWidth()) {
				if (e.type=='mousedown' && !$col.length && $target.closest('.test-plugin-cols-wrap').length ) {	
		...
		$TestPluginAdmin.on('focusin focusout', '.gwa-abox-content input[type="text"], .gwa-abox-content select, .gwa-abox-content textarea, .gwa-abox-content .gwa-checkbox, .gwa-abox-content .gwa-colorpicker, .gwa-abox-content .gwa-checkbox-list', function(e) {
			
			if ($TestPluginAdmin.find('.gwa-pcontent').attr('data-help') != 1 ) return;
	...
	/ ---------------------------------------------------------------------- */
		...
		/* Select icon */
		$TestPluginAdmin.on('click', '[data-action="ip-bg-switch"], [data-action="ip-select"], [data-action="ip-select-blank"], [data-action="ip-select-custom"]', function(e) {
			var $this = $(this),
		...
		$TestPluginAdmin.on('keydown','[data-action="ip-search"] input[type="text"]', function(e) {
		...
		$TestPluginAdmin.on('click','[data-action="ip-search"] a', function(e) {
		...
		$TestPluginAdmin.on('mouseenter','a[data-action="ip-select"]', function(e) {			
		...
		/* Filter select event */		
		$TestPluginAdmin.on('change','[data-action="ip-filter"]', function(e) {
	...
	/ ---------------------------------------------------------------------- */	
		...
		$TestPluginAdmin.on('click', '.gwa-icon-btn a', function(e) {
	...
	/ ---------------------------------------------------------------------- */		
		...
		$TestPluginAdmin.on('click', '.gwa-abox label', function(e) {
	...
	/ ---------------------------------------------------------------------- */
		...
		$TestPluginAdmin.on('change', '.gwa-checkbox input[type=checkbox]', function(e) {
	...
	/ ---------------------------------------------------------------------- */
		...
		/* Checkbox list */
		$TestPluginAdmin.on('click', '.gwa-checkbox-list .gwa-checkbox, .gwa-checkbox-list-toggle', function(e, isTriggered) {
	...
	/ ---------------------------------------------------------------------- */
		...
		function getFavColors() {
			...
			for (var x = 0;  x < cookies.length; x++) {				
				...
				if (cookie.key == 'test_plugin[colors]['+userSettings.uid+']') {
		...
		function addFavColor(color, $elem) {
			...
			if (colors.length) {
				document.cookie='test_plugin[colors]['+userSettings.uid+']='+colors.join('|');
				showFavColors($elem);
			}
	...
	/ ---------------------------------------------------------------------- */
		...
		$TestPluginAdmin.on('focusin focusout', '.gwa-colorpicker input[type=text], .gwa-colorpicker', function(e) {
		...
		$TestPluginAdmin.on('change keydown', '.gwa-cp-popup input', function(e) {
		...
		$TestPluginAdmin.on('click', 'a[data-action="cp-fav"]', function(e) {
			var $this = $(this), $parent = $this.closest('.gwa-input-btn'), $input = $parent.find('input');
			addFavColor($input.val(), $parent);
			e.preventDefault();
		});

		$TestPluginAdmin.on('click', '.gwa-cp-favcolors > span', function(e) {
	...
	/ ---------------------------------------------------------------------- */	
		...
		function showPopupOverlay() {
			...
			var wrapBeforeWidth = $TestPluginAdmin.outerWidth(true),
				$body = $('body'),
				$editorPopup = $('#gwa-editor-popup-wrap');
			
			$body.css('overflow-y', 'hidden');
			var	widthDiff = $TestPluginAdmin.outerWidth(true)-wrapBeforeWidth;
			...
			$TestPluginAdmin.prepend('<div id="gwa-popup-wrap"></div><div id="gwa-popup-overlay"></div>');
	...
	/ ---------------------------------------------------------------------- */			
		...
		$TestPluginAdmin.on('click', '[data-popup="live-preview-edit"]', function(e) {
			
			$(this).data('popup-subtitle', $TestPluginAdmin.find('[name="name"]').val());
			
		});
				

				
		$TestPluginAdmin.on('click', '[data-action="popup"]', function(e) {
			...
			popupXHR = $.ajax({  
					...
					data: $.param({ 
						...
						_nonce : $TestPluginAdmin.find("#_nonce").val(), 
					...
					if (textStatus != 'abort') showAjaxMsg('<p><strong>'+$TestPluginAdmin.data('ajaxerror')+'</strong></p>', 'error');
				}).done(function(data, textStatus, jqXHR) {
	...
	/ ---------------------------------------------------------------------- */				
		...
		$TestPluginAdmin.on('click', '.gwa-popup-close', function(e) {
	...
	/ ---------------------------------------------------------------------- */	
		...
		$TestPluginAdmin.on('click', 'a[data-view]', function(e) {
			...
			document.cookie='test_plugin[settings][popup]['+userSettings.uid+']='+$this.data('view');
			
		});		
		

		/* Create export popup */
		$TestPluginAdmin.on('click', '[data-action="popup"][data-popup="export"]', function(e) {
			$(this).data('id', $('#test-plugin-tm-select').val());
		});
		
		/* Popup edit button event */
		$TestPluginAdmin.on('click', '[data-action="popup-edit"]', function(e) {
			
			var $this = $(this), $popup = $('#gwa-popup-wrap');
			$('#test-plugin-tm-select').val($this.data('id'));		
			$TestPluginAdmin.find('[name="_action_type"]').val('edit').end().find('form').submit();

		});	
		
		/* Popup export button event */
		$TestPluginAdmin.on('click', '[data-action="popup-export"]', function(e) {
			
			var $this = $(this);
			$('#test-plugin-tm-select').val($this.data('id'));
			
			if ($TestPluginAdmin.find('[name="_action_type"]').length) {
				$TestPluginAdmin.find('[name="_action_type"]').val('export').end().find('form').submit();
			} else {
				$('<input />', { 'type' : 'hidden', 'id' : 'action-type', 'name' : '_action_type', 'value' : 'export' }).prependTo($TestPluginAdmin.find('form'));
				$TestPluginAdmin.find('[name="_action"]').val('table_manager').end().find('form').submit();
				/* restore original values */
				$TestPluginAdmin.find('[name="_action"]').val('table_editor');
				$TestPluginAdmin.find('[name="_action_type"]').remove();
			}
	...
	/ ---------------------------------------------------------------------- */			
		...
		$TestPluginAdmin.on('click', 'a[data-action="insert-sc"]', function(e) {
			var $this = $(this),
			...
			if ($popupTarget === undefined || !$TestPluginAdmin.find($popupTarget).length) return;
	...
	/ ---------------------------------------------------------------------- */	
		...
		$TestPluginAdmin.on('click', 'a[data-action="edit-box"]', function(e) {
			...
			editorPopupXHR = $.ajax({  
					...
					data: $.param({ 
						...
						col_type: $this.closest('.test-plugin-col').find('[name*="[col-style-type]"]').find('option').eq($this.closest('.test-plugin-col').find('[name*="[col-style-type]"]').prop('selectedIndex')).data('type'),
						skin : $TestPluginAdmin.find("#test-plugin-style").val(),
	...
	$TestPluginAdmin.on('change', '#gwa-editor-popup-wrap select[data-title="type"]', function(e) {
	...
	$TestPluginAdmin.on('click', 'a[data-action="next"], a[data-action="prev"]', function(e) {
	...
	$TestPluginAdmin.on('change', '[name*="[col-style-type"]', function(e) {
	...
	$TestPluginAdmin.on('click', '[data-action="clone-data"], [data-action="delete-data"], [data-action="add-new-row"]', function(e) {
	...
	$TestPluginAdmin.on('mouseenter mouseleave', '.gwa-col-box-help-icon', function(e) {
	...
	$TestPluginAdmin.on('mouseenter mouseleave dblclick', '.gwa-col-box', function(e) {
		...
		switch(e.type) {
			...
			case 'mouseenter' :
				...
				if (($this.closest('.test-plugin-cols').find('.ui-sortable-helper').length && $this.hasClass('ui-sortable-helper')) || !$this.closest('.test-plugin-cols').find('.ui-sortable-helper').length) {
	...
	$TestPluginAdmin.on('click', '.gwa-col-box-link', function(e) {
		...
		function showEditorPopup(data) {
			...
			if (!$popup.length) {
				firstLoad = true;
				$popup = $('<div id="gwa-editor-popup-wrap"></div>').prependTo($TestPluginAdmin);
			} else {
		...
		$TestPluginAdmin.on('click', '[data-action="insert-data"]', function(e) {
	...
	/* Popup tabs */
	$TestPluginAdmin.on('click', '.gwa-popup-tab', function() {
	...
	$TestPluginAdmin.find('.gwa-checkbox input[type=checkbox]').trigger('change');
	...
	var $CodeEditors = $TestPluginAdmin.find('.gwa-textarea-code textarea');
	
	$TestPluginAdmin.on('focusin focusout', '.CodeMirror', function(e) {
	...
	$TestPluginAdmin.on('click', '.gwa-abox-header-tab', function() {
		
		var $this = $(this),
		codeEditor = $this.closest('.gwa-abox').find('.gwa-abox-tab-content.gwa-current .gwa-textarea-code textarea');
		$this.closest('.gwa-abox').find('.gwa-abox-tab-content.gwa-current').find('select, input[type=checkbox]').not('#test-plugin-style').trigger('change');
	...
	$TestPluginAdmin.on('click', 'input[readonly]', function() {
		$(this).select();
	});
	...
	$TestPluginAdmin.on('click', '[data-action="clipboard-prev"]', function(e) {

		var $this = $(this),
			$clipboard = $this.closest('#gwa-editor-popup-wrap').length ? $this.closest('.gwa-popup-content-wrap').find('.gwa-clipboard-wrap-preview') : $this.closest('#test-plugin-column-editor').find('.gwa-clipboard-wrap-preview');
	...
	$TestPluginAdmin.on('click', '.gwa-message-close', function(e) {

		var $this = $(this),
			$message = $this.closest('.gwa-message, .gwa-banner'),
			id = $message.data('id') || 0;

		document.cookie='test_plugin_msg_'+id+' = true; expires=Fri, 31 Dec 9999 23:59:59 GMT';
  		$message.slideUp();
