...
class DX_TestPlugin_Admin {	
	...
	public function __construct( $globals ) {
		...
		// Handle ajax actions
		add_action( 'wp_ajax_test_plugin_ajax_action', array( __CLASS__, 'ajax_action_router' ) );
		add_action( 'wp_ajax_nopriv_test_plugin_ajax_action', array( __CLASS__, 'nopriv_ajax_request' ) );		
		
		// Clip ad
		add_action( 'test_plugin_init', array( $this, 'clip_ad' ), 9999 );
	...
	public function clip_ad() {
		...
		if ( class_exists( 'DX_TestPlugin_Clip' ) ) return;	
		add_action( 'test_plugin_editor_popup_content_before_html', array( $this, 'editor_popup_clipboard_html' ) );
		add_action( 'test_plugin_colum_editor_content_before_html', array( $this, 'clipboard_html' ) );
		add_action( 'test_plugin_colum_editor_nav_html', array( $this, 'clipboard_nav_html' ) );
	...
	public function editor_popup_clipboard_html( $data ) {
		...
		<div class="gwa-clipboard-wrap-preview"<?php echo isset( $data['_action_type'] ) ? sprintf( ' data-clip-id="%s"', esc_attr( $data['_action_type'] ) ) : ''; ?>>
			<div class="gwa-clipboard-preview">
				<div class="gwa-clipboard-message-prev"><p><strong>Easier and faster work? Of course!</strong><br>By using ‘Clip – Add-on for Go Pricing’ you can copy this popup content within a table or between tables.</p><a href="http://codecanyon.net/item/clip-addon-for-test-plugin/13216605?ref=Granth" target="_blank" title="<?php esc_attr_e( 'Buy Now', 'test_plugin_clip_textdomain' ); ?>" class="gwa-btn-style2"><?php _e( 'Buy Now', 'test_plugin_clip_textdomain'); ?></a></div>
				<div class="gwa-clip-assets-nav-preview"><a href="#" class="gwa-asset-icon-clipboard-preview" data-action="clipboard-prev" title="<?php esc_attr_e( 'Open / Close Clipboard', 'test_plugin_clip_textdomain' ); ?>"><span></span></a></div>
				<a href="#" title="<?php esc_attr_e( 'Close', 'test_plugin_clip_textdomain' ); ?>" class="gwa-clipboard-close"></a>
			</div>
		</div>
		<?php
		echo ob_get_clean();
	...
	public function clipboard_nav_html( $data ) {
		...
		ob_start();
		?>
		<a href="#" data-action="clipboard-prev" title="<?php _e( 'Open / Close Clipboard', 'test_plugin_clip_textdomain' ); ?>" class="gwa-abox-header-nav-clip-preview"><?php _e( 'Clip', 'test_plugin_clip_textdomain' ); ?></a>
		<?php
		echo ob_get_clean();
	...
	public function check_db_version() {
		...
		if ( ( empty( $version_info['db'] ) || ( !empty( $version_info['db'] ) && version_compare( $version_info['db'], self::$db_version, "<" ) ) ) && !empty( $old_db_tables ) ) {
			add_action( 'test_plugin_menu_pages', array($this, 'register_db_update_page' ));
			add_filter( 'test_plugin_page_router_callback', array( $this, 'force_db_update_page' ) );
		}
	...
	public function register_db_update_page() {
		...
		self::add_submenu_page(
			__( 'Database Update', 'test_plugin_textdomain' ),
			__( 'Database Update', 'test_plugin_textdomain' ),
	...
	public function load_includes() {
		...
		DX_TestPlugin_AdminNotices::instance();
		...
		DX_TestPlugin_VCExtend::instance();
	...
	public function enqueue_admin_styles() {
		...
		self::$screen_hooks = apply_filters( 'test_plugin_add_screen', self::$screen_hooks );
		...
		if ( array_key_exists( $screen->id, self::$screen_hooks ) ) {
			...
			wp_enqueue_style( self::$plugin_slug . '-admin-styles', $this->plugin_url . 'assets/admin/css/test_plugin_admin_styles.css', array(), self::$plugin_version );
	...
	public function enqueue_admin_scripts() {
		...
		if ( array_key_exists( $screen->id, self::$screen_hooks ) ) {
			...
			wp_enqueue_script( self::$plugin_slug . '-admin-scripts', $this->plugin_url . 'assets/admin/js/test_plugin_admin_scripts.js', array( 'jquery' ), self::$plugin_version );						
	...
	public function localization() {
		...
		$translate = array(
			'ajax_error' => __( 'PHP Error in AJAX Response!', 'test_plugin_textdomain' ),
			'upload_size' => __( 'File size exceeds the maximum upload size!', 'test_plugin_textdomain' ),
			'upload_size_null' => __( 'File is emtpy!', 'test_plugin_textdomain' ),			
			'upload_ext' => __( 'Invalid file type', 'test_plugin_textdomain' ),			
			'warning_maxcol' => __( 'You have reached the maximum number of columns!', 'test_plugin_textdomain' ),
			'warning_invalid_imge' => __( 'Invalid image!', 'test_plugin_textdomain' )
		);					
		
		wp_localize_script( self::$plugin_slug . '-admin-scripts', 'TestPluginL10n', $translate );	
	...
	public function register_menu_pages() {
		...
		self::add_submenu_page(
			__( 'General Settings', 'test_plugin_textdomain' ),
			__( 'General Settings', 'test_plugin_textdomain' ),
			isset( $general_settings['capability'] ) ? $general_settings['capability'] : 'manage_options', 
		...
		// Import & Export page
		self::add_submenu_page(
			__( 'Import & Export', 'test_plugin_textdomain' ),
			__( 'Import & Export', 'test_plugin_textdomain' ),
			isset( $general_settings['capability'] ) ? $general_settings['capability'] : 'manage_options', 
		...
		// Add-ons page
		self::add_submenu_page(
			__( 'Add-ons', 'test_plugin_textdomain' ),
			__( 'Add-ons', 'test_plugin_textdomain' ),
			isset( $general_settings['capability'] ) ? $general_settings['capability'] : 'manage_options', 
		...
		if ( !is_multisite() || ( is_multisite() && get_current_blog_id() == 1 ) ) {
			...
			// Plugin Update page
			self::add_submenu_page(
				__( 'Update', 'test_plugin_textdomain' ),
				__( 'Update', 'test_plugin_textdomain' ),
				$capability, 
		...
		do_action( 'test_plugin_menu_pages', self::$screen_hooks );	
	...
	public static function main_page() {
		...
		$page = new DX_TestPlugin_AdminPage_Main( __METHOD__ );
	...
	public static function settings_page() {
		...
		$page = new DX_TestPlugin_AdminPage_GeneralSettings( __METHOD__ );
		$page->title( sprintf( 'Test Plugin - %s',  __( 'General Settings', 'test_plugin_textdomain' ) ) );
		echo $page->render();
	...
	public static function impex_page() {
		...
		$page = new DX_TestPlugin_AdminPage_Impex( __METHOD__ );
		$page->title( sprintf( 'Test Plugin - %s',  __( 'Import & Export', 'test_plugin_textdomain' ) ) );
		echo $page->render();	
	...
	public static function addons_page() {
		...
		$page = new DX_TestPlugin_AdminPage_Addons( __METHOD__ );
		$page->title( sprintf( 'Test Plugin - %s',  __( 'Add-ons', 'test_plugin_textdomain' ) ) );
		echo $page->render();
	...
	public static function plugin_updater_page() {
		...
		$page = new DX_TestPlugin_AdminPage_Update( __METHOD__ );
		$page->title( sprintf( 'Test Plugin - %s',  __( 'Update', 'test_plugin_textdomain' ) ) );
		echo $page->render();
	...
	public static function db_updater_page() {
		...
		$page = new DX_TestPlugin_AdminPage_DbUpdate( __METHOD__ );
		$page->title( sprintf( 'Test Plugin - %s',  __( 'Database Update', 'test_plugin_textdomain' ) ) );
		echo $page->render();
	...
	public static function admin_page_router() {
		...
		if ( !empty( $screen->id ) && !empty( self::$screen_hooks[$screen->id] ) ) {
			...
			// Global filter
			$page_callback = apply_filters( 'test_plugin_page_router_callback', $page_callback );
	...
	public static function ajax_action_router() {		
		...
		if ( empty( $_POST['action'] ) || empty( $_POST['_action'] ) ) {
			...
			DX_TestPlugin_AdminNotices::add( 'ajax', 'error', __( 'No AJAX action has been found!', 'test_plugin_textdomain' ) );
		...
		if ( empty( $actions['data'] ) || empty( $actions['token'] ) || sha1( @serialize( $actions['data'] ) ) != $actions['token'] ) {
			...
			DX_TestPlugin_AdminNotices::add( 'ajax', 'error', __( 'Invalid AJAX callback has been detected! Please, refresh the page!', 'test_plugin_textdomain' ) );
			DX_TestPlugin_AdminNotices::show();
			exit;
		...
		if ( !empty( $actions['data'][$_POST['_action']] ) ) {
			...
			DX_TestPlugin_AdminNotices::add( 'ajax', 'error', __( 'No AJAX callback has been registered for this action! Please, refresh the page!', 'test_plugin_textdomain' ) );
			DX_TestPlugin_AdminNotices::show();
			exit;			
