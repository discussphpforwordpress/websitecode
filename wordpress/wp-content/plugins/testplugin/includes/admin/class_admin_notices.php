...
// Class
class DX_TestPlugin_AdminNotices {
	...
	public static function print_remote_admin_notices() {
		...
		if ( empty( $screen ) || empty( $screen->id ) || preg_match('/^test-plugin_page|toplevel_page_test-plugin/', $screen->id ) != 1 ) return;
		
		$apicall = new DX_TestPlugin_Api( array( 'product' => 'test_plugin', 'type' => 'message' ) );
		$api_data = $apicall->get_data();

		if ( empty( $api_data ) ) return;
		
		/* Defaults */
		$cid_base = 'test_plugin_msg_';
		$allowed_html = array(
			'a' => array(
				'href' => array(),
				'title' => array(),
		...
		if ( !empty( $api_data ) && is_array( $api_data ) ) {
			...
			foreach ( $api_data as $api_message ) {
				...
				/* Validate page */
				if ( $api_message['target'] == 'dashboard' && preg_match('/^toplevel_page_test-plugin/', $screen->id ) != 1 ) continue;
				...
				printf( '<div id="message" data-id="%1$s" class="%2$s"%3$s>%4$s<a href="#" class="gwa-message-close" title="%5$s"></a></div>', 
					...
					$msg, 
					esc_attr__( 'Close', 'test_plugin_textdomain' )
				);				
