...
// Class
class DX_TestPlugin_AdminPage_Main extends DX_TestPlugin_AdminPage {
	...
	public function register_ajax_actions( $ajax_action_callback ) { 
		...
		DX_TestPlugin_Admin::register_ajax_action( 'table_manager', $ajax_action_callback );
		DX_TestPlugin_Admin::register_ajax_action( 'table_editor', $ajax_action_callback );	
		DX_TestPlugin_Admin::register_ajax_action( 'table_column', $ajax_action_callback );	
		DX_TestPlugin_Admin::register_ajax_action( 'table_row', $ajax_action_callback );
		DX_TestPlugin_Admin::register_ajax_action( 'table_button', $ajax_action_callback );	
		DX_TestPlugin_Admin::register_ajax_action( 'popup', $ajax_action_callback );
		DX_TestPlugin_Admin::register_ajax_action( 'editor_popup', $ajax_action_callback );
		DX_TestPlugin_Admin::register_ajax_action( 'editor_columns', $ajax_action_callback );
		DX_TestPlugin_Admin::register_ajax_action( 'export', $ajax_action_callback );		
		...
		if ( empty( $this->action ) ) {
			...
			switch ( $action ) {
				...
				case 'edit':
					...
					$result = DX_TestPlugin_Data::get_table( (int)$_GET['id'] );
			...
			switch( $this->action ) {
						...
						switch( $this->action_type ) {
							...
							case 'create':								
								...
								if ( $this->is_ajax === false ) {
									wp_redirect( ( add_query_arg( 'action', 'create', 'admin.php?page=test-plugin' ) ) );	
									exit;
								} else {
							...
							case 'edit':
								...
								if ( $result === false ) {
									...
									// Redirect / Load view
									if ( $this->is_ajax === false ) {
										wp_redirect( ( add_query_arg( array( 'action' => 'edit', 'id' => $_POST['postid'] ), admin_url( 'admin.php?page=test-plugin' ) ) ) );	
										exit;
									} else {																			
										echo $this->view( 'table_editor' );
									}
							...
							case 'order':
								...
								if ( !empty( $_POST['_order'] ) && !empty( $_POST['_orderby'] ) ) {
									...
									switch ( $_POST['_order'] ) { 
										case 'ASC' : 
										case 'DESC' : 
											setcookie( "test_plugin[settings][tm][order][$user_id]", $_POST['_order'] );
											break;
									...
									switch ( $_POST['_orderby'] ) { 
										...
										case 'date' :
										case 'modified' :
											setcookie( "test_plugin[settings][tm][orderby][$user_id]", $_POST['_orderby'] );
											break;
							...
							case 'export':
								...
								if ( $result === false ) {
									...
									if ( $this->is_ajax === false ) {
										wp_redirect(  add_query_arg( array( 'action' => 'export' ), admin_url( 'admin.php?page=test-plugin-import-export' ) ) );	
										exit;
									} else {
				...
				case 'table_editor':
					...
					if ( $result === false ) { 
						...
						if ( $this->is_ajax === false ) {
							wp_redirect( add_query_arg( array( 'action' => 'edit', 'id' => $result ), admin_url( 'admin.php?page=test-plugin' ) ) );	
							exit;
						} else {
	...
	public function editor_popup() {
		...
		if (isset( $postdata ) ) $postdata = DX_TestPlugin_Helper::clean_input( $postdata, 'html' );

		ob_start();	
		$file = "views/editor_popup/{$this->action_type}.php";
		$file = apply_filters( "test_plugin_admin_editor_popup_{$this->action_type}", $file );
		include_once( $file );	
	...
	public function validate_edit( $data ) { 
		...
		if ( empty( $data ) ) {
			...
			DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Please select a table!', 'test_plugin_textdomain' ) );
			return false;
			
		} else {
		
			$result = DX_TestPlugin_Data::get_table( (int)$data );

			if ( $result === false ) {
				DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Table doesn\'t exist!', 'test_plugin_textdomain' ) );
				return false;
			}
	...
	public function copy_table( $data ) { 
		...
		$data = DX_TestPlugin_Helper::clean_input( $data );
		...
		foreach ( (array)$table_ids as $table_id ) {
			...
			$table_data = DX_TestPlugin_Data::get_table( (int)$table_id );

			if ( !empty( $table_data ) ) {
				
				if ( !empty( $table_data['postid'] ) ) unset( $table_data['postid'] );
				$table_data['name'] .= ' (' . __( 'copy', 'test_plugin_textdomain' ) . ')';	
				$table_data['id'] = substr( $table_data['id'], 0 ,10 ) . '_' . uniqid();

				$result = DX_TestPlugin_Data::update_table( $table_data );
				$cnt++; 
		...
		$notices = DX_TestPlugin_AdminNotices::get( 'main', 'error' );
		
		if ( empty( $notices ) ) DX_TestPlugin_AdminNotices::add( 'main', 'success', sprintf( __( '%1$s pricing table(s) has been successfully cloned.', 'test_plugin_textdomain' ), $cnt ) );
	...
	public function delete_table( $data ) { 
		...
		foreach ( (array)$table_ids as $table_id ) {
			...
			$result = DX_TestPlugin_Data::delete_table( (int)$table_id );
			$cnt++;
		...
		if ( empty( $notices ) ) DX_TestPlugin_AdminNotices::add( 'main', 'success', sprintf( __( '%1$s pricing table(s) has been successfully deleted.', 'test_plugin_textdomain' ), $cnt ) );
	...
	public function save_table( $data ) { 
		...
		if ( empty( $data['name'] ) ) DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Pricing "Table Name" is empty!', 'test_plugin_textdomain' ), false );		
		
		if ( empty( $data['id'] ) ) {
			DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Pricing "Table ID" is empty!', 'test_plugin_textdomain' ), false );

		} elseif ( strlen( $data['id'] ) >= 1 && preg_match( '/([^a-z0-9\-_])+/' , $data['id'] ) == 1 ) {
			DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Pricing "Table ID" can contain only lowercase letters, numbers, hyphens and underscores!', 'test_plugin_textdomain' ), false );
		} else {
			$tables = DX_TestPlugin_Data::get_tables();
			...
			if ( in_array( $data['id'], $table_ids ) ) DX_TestPlugin_AdminNotices::add( 'main', 'error', sprintf( __( 'Pricing table with an ID of "%s" already exists! It must be uniqe.', 'test_plugin_textdomain' ), $data['id'] ), false );
		}
				
		$notices = DX_TestPlugin_AdminNotices::get( 'main', 'error' );
		
		// Exclude col data
		if ( isset( $data['col-data'] ) ) $col_data = DX_TestPlugin_Helper::clean_input( $data['col-data'], 'html' ); 

		// Clean custom CSS
		if ( isset( $data['custom-css'] ) ) {
			$custom_css = DX_TestPlugin_Helper::clean_input( array( $data['custom-css'] ), 'no_html' );
			$data['custom-css'] = $custom_css[0];
		}		
		
		// Clean postdata (the rest)
		$data = DX_TestPlugin_Helper::clean_input( $data, 'filtered', '', array( 'custom-css' ) );

		if ( isset( $col_data ) ) $data['col-data'] = $col_data;	
		
		if ( empty( $notices ) ) {

			$data = DX_TestPlugin_Helper::remove_input( $data, array( $_POST['action'] ) );
			$postid = DX_TestPlugin_Data::update_table( $data );
			
			if ( $postid !== false ) {
				
				// Create table message
				if ( empty( $data['postid'] ) ) {
					DX_TestPlugin_AdminNotices::add( 'main', 'success', sprintf( __( 'Pricing table has been successfully created!<br>Shortcode: </strong>[test_plugin id="%s"]<strong>', 'test_plugin_textdomain' ), $data['id'] ) );
				} else {
					DX_TestPlugin_AdminNotices::add( 'main', 'success', sprintf( __( 'Pricing table has been successfully updated!<br>Shortcode: </strong>[test_plugin id="%s"]<strong>', 'test_plugin_textdomain' ), $data['id'] ) );
				}
				return $postid;
			} else {
				DX_TestPlugin_AdminNotices::add( 'main', 'error', __( 'Oops, something went wrong!', 'test_plugin_textdomain' ) );	
	...
	public function get_editor_columns() {
		...
		$table_data = !empty( $_POST['postid'] ) ? DX_TestPlugin_Data::get_table( (int)$_POST['postid'] ) : null;		
	...
	public function get_global_style_opts() {
		...
		echo '<div id="test-plugin-global-style">';
		do_action( "test_plugin_admin_global_style_opts_{$style}", !empty( $table_data ) ? $table_data : '' );
		echo '</div>';		 
	}	
	...
	public function load_popup() { 
		...
		if ( !empty( $this->action_type ) ) {
			...
			do_action( "test_plugin_popup_before_{$this->action_type}" );
			...
			do_action( "test_plugin_popup_after_{$this->action_type}" );
