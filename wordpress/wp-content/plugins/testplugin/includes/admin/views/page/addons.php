...
<!-- Page Content -->
<div class="gwa-pcontent" data-help="<?php echo esc_attr( isset( $_COOKIE['test_plugin']['settings']['help'][$user_id] ) ? $_COOKIE['test_plugin']['settings']['help'][$user_id] : '' ); ?>">
		...
		<div class="gwa-abox">
			<div class="gwa-abox-header">
	            <div class="gwa-abox-header-tab gwa-current">
					...
					<div class="gwa-abox-title"><?php _e( 'Plugin Add-ons', 'test_plugin_textdomain' ); ?></div>
				</div>
				...
				<div class="gwa-abox-content">
                    <?php 
						if ( empty( $api_data ) || empty( $api_data['addons'] ) ) {
							printf( '<div style="margin:10px;"><strong>%s</strong></div>', __( 'Oops, API data is not available, please try again later!', 'test_plugin_textdomain' ) );			
							...
							foreach( $api_data['addons'] as $addon ) {
								...
								if ( !empty( $addon['name'] ) ) {
									...
									if ( !empty( $addon['version'] ) ) {
										$metadata[] = array( 
											'label' => __( 'Latest Version' , 'test_plugin_textdomain'), 
											'data' => $addon['version']
										);
									}
									...
									if ( !empty( $addon['core_min'] ) ) {
										$metadata[] = array( 
											'label' => __( 'Test Plugin Requirement' , 'test_plugin_textdomain'), 
											'data' => $addon['core_min'] . '+'
										);
									}
									...
									$item_main_footer = sprintf( '<div class="gwa-product-footer">%s</div>',
										!empty( $addon['purchase_url'] ) ? sprintf( '<a href="%1$s" class="gwa-btn-style2" target="_blank">%2$s</a>', 
											$addon['purchase_url'],
											__( 'Buy Now' , 'test_plugin_textdomain')
										) : ''
									);
