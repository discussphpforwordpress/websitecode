...
<div class="gwa-pcontent" data-ajax="<?php echo esc_attr( isset( $general_settings['admin']['ajax'] ) ? "true" : "false" ); ?>" data-help="<?php echo esc_attr( isset( $_COOKIE['test_plugin']['settings']['help'][$user_id] ) ? $_COOKIE['test_plugin']['settings']['help'][$user_id] : '' ); ?>">
	<form id="test-plugin-form" name="import-form" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
			...
			<div class="gwa-abox-content-wrap">
				<div class="gwa-abox-content">
					<table class="gwa-table">							
						<tr class="test-plugin-group" data-parent="import-export" data-children="export">
							<th><label><?php printf( __( 'Select Tables%s', 'test_plugin_textdomain' ), sprintf( ' <span class="gwa-info">(%d)</span>', is_array( $data['data'] ) ? count( $data['data'] ) : 0 ) ); ?></label></th>
							<td>
							<?php if ( !empty( $data['data'] ) ) : ?>
							<ul class="gwa-checkbox-list">
								<li><label><span class="gwa-checkbox gwa-checked" tabindex="0"><span></span><input type="checkbox" name="import[]" value="all" checked="checked" class="gwa-checkbox-parent"></span><?php _e( 'All tables', 'test_plugin_textdomain' ); ?></label><span class="gwa-checkbox-list-toggle"></span>
									<ul class="gwa-checkbox-list">
							...
							<td class="gwa-abox-info"><p class="gwa-info"><i class="fa fa-info-circle"></i><?php _e( 'Select the pricing tables you would like to import.', 'test_plugin_textdomain' ); ?></p></td>
						</tr>
						<tr>
							<th><label><?php _e( 'Replace existing items?', 'test_plugin_textdomain' ); ?></label></strong></th>
							<td><p><label><span class="gwa-checkbox" tabindex="0"><span></span><input type="checkbox" name="replace" value="1"></span><?php _e( 'Yes', 'test_plugin_textdomain' ); ?></label></p></td>
							<td class="gwa-abox-info"><p class="gwa-info"><i class="fa fa-info-circle"></i><?php _e( 'Whether to replace existing items with same ids or create new copy of them.', 'test_plugin_textdomain' ); ?></p></td>									
						</tr>												
					</table>			
				</div>
			 </div>
