...
class DX_TestPlugin_Data {
	...
	protected static $cpt_name = 'Test Plugin Tables';
	protected static $cpt_slug = 'test_plugin_tables';
	...
	private function __construct() {
		...
		self::$globals = DX_TestPlugin::instance();
	...
	public static function export( $post_ids = array() ) {
		...
		$tables_data = apply_filters( 'test_plugin_export_data', $tables_data );
		...
		foreach ( (array)$import_tables_data as $import_tables_key => $import_table_data ) {
			...
			if ( !empty( $table_ids ) && isset( $import_table_data['id'] ) && $import_table_data['id'] != '' && array_key_exists( $import_table_data['id'], (array)$table_ids ) ) {
				...
				if ( $override === true ) {
					...
					$import_table_data['name'] .= ' (' . __( 'copy', 'test_plugin_textdomain' ) . ')';	
	...
	public static function db_update() {
		...
		$old_tables_data = get_option( 'test_plugin_tables' );		
		$general_settings = get_option( 'test_plugin_table_settings' );
