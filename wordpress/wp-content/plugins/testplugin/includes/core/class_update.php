...
class DX_TestPlugin_Update {
	...
	public function check_update( $transient ) {
		...
		$api_products['test_plugin'] = array(
			'id' => 'test_plugin',
		...
		if ( !empty( $test_plugin['addons'] ) && !empty( $api_data['addons'] ) ) {
		...
		$api_products['test_plugin']['api_data'] = $api_data;
		
		if ( !empty( $test_plugin['addons'] ) ) $api_products = array_merge( $api_products, $test_plugin['addons'] );
	...
	public function update_info( $false, $action, $args ) {
		...
		$apicall = new DX_TestPlugin_Api( array( 'product' => $update_data[$slug]['id'], 'type' => 'log' ) );
		$api_data = $apicall->get_data();		
		...
		if ( !empty( $api_data ) ) {
			...
			foreach( $api_data as $version => $info ) {
				...
				$more_info = !empty( $info['details'] ) ? sprintf( ' - <a href="%1$s" target="_blank">%2$s</a>', $info['details'], __( 'More info', 'test_plugin_textdomain' ) ) : ''; 
				$change_log .= '<em>' . sprintf( '%1$s - %2$s', __( 'Release Date', 'test_plugin_textdomain' ), date_i18n( get_option( 'date_format' ), strtotime( $info['date'] ) ) ) . $more_info . '</em>';
