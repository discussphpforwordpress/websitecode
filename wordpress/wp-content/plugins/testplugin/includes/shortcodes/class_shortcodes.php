...
class DX_TestPlugin_Shortcodes {
	...
	private function __construct( $globals ) {
		...
		/* Main shortcode */				
		add_shortcode( 'test_plugin', array( $this, 'pricing_table_sc' ) );
		
		/* Video shortcodes */
		add_shortcode( 'test_plugin_html5_video', array( $this, 'video_sc' ) );
		add_shortcode( 'test_plugin_youtube', array( $this, 'youtube_sc' ) );
		add_shortcode( 'test_plugin_vimeo', array( $this, 'vimeo_sc' ) );
		add_shortcode( 'test_plugin_screenr', array( $this, 'screenr_sc' ) );
		add_shortcode( 'test_plugin_dailymotion', array( $this, 'dailymotion_sc' ) );
		add_shortcode( 'test_plugin_metacafe', array( $this, 'metacafe_sc' ) );		

		/* Audio shortcodes */
		add_shortcode( 'test_plugin_audio', array( $this, 'audio_sc' ) );
		add_shortcode( 'test_plugin_soundcloud', array( $this, 'soundcloud_sc' ) );
		add_shortcode( 'test_plugin_mixcloud', array( $this, 'mixcloud_sc' ) );
		add_shortcode( 'test_plugin_beatport', array( $this, 'beatport_sc' ) );
		
		/* Mixed */
		add_shortcode( 'test_plugin_map', array( $this, 'goole_map_sc' ) );
		add_shortcode( 'test_plugin_iframe', array( $this, 'iframe_sc' ) );
	...
	/**
	 * Pricing table shortcode [test_plugin]
	 *
	...
	public function pricing_table_sc( $atts, $content = null ) {
		...
		if ( empty( $id ) && empty( $postid ) && empty( $data ) ) return '<p>' . __( 'You must set a Table ID or Post ID.', 'test_plugin_textdomain' ) . '</p>';
		...
		if ( !empty( $postid ) ) {
			...
			$postid = (int)$postid;
			$pricing_table = DX_TestPlugin_Data::get_table( $postid );

		} else {
			
			$id = sanitize_key( $id );
			
			$pricing_table = DX_TestPlugin_Data::get_table( $id, true, 'id' );
		...
		if ( empty( $pricing_table ) && empty( $data ) ) return '<p>' . sprintf( __( 'Pricing table with an %1$s of "%2$s" is not defined.', 'test_plugin_textdomain' ), ($id_table) ? __('Table ID', 'test_plugin_textdomain' ) : __('Post ID', 'test_plugin_textdomain' ), !empty( $id ) ? $id : $postid ) . '</p>';		
				
		if ( empty( $pricing_table['col-data'] ) && $preview  == 'true' ) return '<div id="test-plugin-forbidden"><i class="icon-exclamation-triangle"></i>'. __( 'There is nothing to see here, yet. Please, first create columns!', 'test_plugin_textdomain' ) . '</div>';
		...
		$all_styles = !empty( $test_plugin['styles'] ) ? $test_plugin['styles'] : array();
		...
		$main_classes = apply_filters( 'test_plugin_front_main_classes', $main_classes, $pricing_table );
		$main_styles = apply_filters( 'test_plugin_front_man_styles', $main_styles, $pricing_table );				
		
		// Build html
		$main_head_html = sprintf( 
			'<div id="test-plugin-table-%1$d" class="test-plugin"%2$s>', 
			$postid, 
			!empty( $main_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$main_styles )  ) .  ';"' : ''
		);
		
		$main_head_html .= sprintf( 
			'<div%1$s data-id="%2$d" data-colnum="%3$d" data-equalize="%4$s"%5$s%6$s%7$s%8$s>', 
			!empty( $main_classes) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$main_classes )  ) .  '"' : '', 
			!empty( $pricing_table['postid'] ) ? (int)$pricing_table['postid'] : '',
			$colnum,
			!empty( $pricing_table['equalize'] ) ? DX_TestPlugin_Helper::esc_attr( json_encode( $pricing_table['equalize'] ) ) : '',
			!empty( $pricing_table['responsivity']['enabled'] ) && !empty( $pricing_table['responsivity']['views'] ) ? ' data-views="' . DX_TestPlugin_Helper::esc_attr( json_encode( $pricing_table['responsivity']['views'] ) ) . '"' : '',
			!empty( $pricing_table['col-animation'] ) && isset( $pricing_table['col-anim-offset'] ) ? ' data-scroll-offset="' . DX_TestPlugin_Helper::esc_attr( (int)$pricing_table['col-anim-offset'] ) . '"' : '',
			!empty( $pricing_table['col-animation'] ) && !empty( $pricing_table['col-anim-once'] ) ? ' data-anim-repeat="1"' : '',
			!empty( $pricing_table['col-width']['max'] ) && (int)$pricing_table['col-width']['max'] > 0  ? ' data-mw="' . DX_TestPlugin_Helper::esc_attr( (int)$pricing_table['col-width']['max'] ) . '"' : ''
		);
		
		// Filter html
		$main_head_html = apply_filters( 'test_plugin_front_main_head_html', 
			$main_head_html, 
			array(
		...
		foreach ( (array)$pricing_table['col-data'] as $col_index => $col_data ) {
			...
			/* Layout & Style */
			if ( isset( $col_data['layout-style'] ) && is_string( $col_data['layout-style'] ) ) $col_data['layout-style'] = DX_TestPlugin_Helper::parse_data( $col_data['layout-style'] );				 
			...
			/* Decoration */
			if ( isset( $col_data['decoration'] ) && is_string( $col_data['decoration'] ) ) $col_data['decoration'] = DX_TestPlugin_Helper::parse_data( $col_data['decoration'] );			

			/* Header Style */
			if ( isset( $col_data['header']['style'] ) && is_string( $col_data['header']['style'] ) ) $col_data['header']['style'] = DX_TestPlugin_Helper::parse_data( $col_data['header']['style'] );
			
			/* Header General */
			if ( isset( $col_data['header']['general'] ) && is_string( $col_data['header']['general'] ) ) $col_data['header']['general'] = DX_TestPlugin_Helper::parse_data( $col_data['header']['general'] );

			/* Title */
			if ( isset( $col_data['title'] ) && is_string( $col_data['title'] ) ) $col_data['title'] = DX_TestPlugin_Helper::parse_data( $col_data['title'] );

			/* Body row */
			if ( isset( $col_data['body-row'] ) && is_string( $col_data['body-row'] ) ) $col_data['body-row'] = DX_TestPlugin_Helper::parse_data( $col_data['body-row'] );
			...
			if ( !empty( $pricing_table['style'] ) && !empty( $col_data['col-style-type'] ) ) {
				...
				/* Get registered styles */
				$registered_styles = $test_plugin['style_types'][$pricing_table['style']];
			...
			if ( !empty( $column_style_data['type'] ) ) {
				...
				if ( $column_style_data['type'] != 'html' && $column_style_data['type'] != 'chtml' ) {
					...
					if ( $column_style_data['type'] != 'team' && $column_style_data['type'] != 'cteam' ) {
						...
						if ( isset( $col_data['price'] ) && is_string( $col_data['price'] ) ) $col_data['price'] = DX_TestPlugin_Helper::parse_data( $col_data['price'] );
			...
			$col_wrap_classes = apply_filters( 'test_plugin_front_colwrap_classes', 
				$col_wrap_classes, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,					
			...
			$col_wrap_atts = apply_filters( 'test_plugin_front_colwrap_atts', 
				$col_wrap_atts, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,					
			...
			foreach ( (array)$col_wrap_atts as $col_wrap_att ) {
				if ( count( (array)$col_wrap_att ) == 2 ) $col_wrap_attributes[] = sprintf( '%1$s="%2$s"', $col_wrap_att[0], DX_TestPlugin_Helper::esc_attr( $col_wrap_att[1] ) );
			}	
			...
			if ( isset( $test_plugin['column-transition'] ) && !empty( $pricing_table['col-animation'] ) ) {
					
				if ( !empty( $col_data['animation'] ) && is_string( $col_data['animation'] ) ) $col_anim = DX_TestPlugin_Helper::parse_data( $col_data['animation'] );
				...
				if ( isset( $col_anim['transition'] ) && $col_anim['transition'] != '' && !empty( $col_anim['trans']['duration'] ) && isset( $col_anim['trans']['delay'] ) && isset( $col_anim['trans']['ease'] ) ) {
					...
					foreach ( $test_plugin['column-transition'] as $trans ) {
			...
			$col_wrap_header_html = sprintf( '<div%1$s%2$s%3$s>', 
				!empty( $col_wrap_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$col_wrap_classes )  ) .  '"' : '',
				!empty( $col_wrap_attributes ) ? ' ' . implode( ' ', (array)$col_wrap_attributes ) : '',
				!empty( $col_anim_data ) ? sprintf( ' data-col-anim="%s"', DX_TestPlugin_Helper::esc_attr( json_encode( $col_anim_data ) ) ) : ''
			);
			
			// Column wrap - Filter html
			$col_wrap_header_html = apply_filters( 'test_plugin_front_colwrap_head_html', 
				$col_wrap_header_html,
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,					
			...
			$col_classes = apply_filters( 'test_plugin_front_col_classes', 
				$col_classes, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,					
			...
			$col_styles = apply_filters( 'test_plugin_front_col_styles', 
				$col_styles, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$col_header_html = sprintf( 
				...
				!empty( $col_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$col_classes )  ) .  '"' : '',
				!empty( $col_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$col_styles )  ) .  ';"' : ''
			);
								
			// Column wrap - Filter html
			$col_header_html = apply_filters( 'test_plugin_front_col_head_html', 
				$col_header_html,
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,					
			...
			if ( !empty( $col_data['decoration']['col-sign'] ) && !empty( $col_data['decoration']['col-sign-type'] ) ) {			
				...
				if ( $col_data['decoration']['col-sign-type'] == 'text' ) {
					...
					$sign_inner_html = sprintf(	'<span%1$s>%2$s</span>',
						!empty( $sign_span_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$sign_span_styles )  ) .  ';"' : '',
						DX_TestPlugin_Helper::esc_sprint( isset ( $col_data['decoration']['col-sign']['text']['content'] ) ? $col_data['decoration']['col-sign']['text']['content'] : '' )
					 );
					...
					if ( !empty( $col_data['decoration']['col-sign'][$col_data['decoration']['col-sign-type']]['data'] ) ) {
						$sign_inner_html = sprintf( '<img src="%1$s" alt="%2$s">', 
							DX_TestPlugin_Helper::esc_attr( $col_data['decoration']['col-sign'][$col_data['decoration']['col-sign-type']]['data'] ),
							isset( $col_data['decoration']['col-sign-alt'] ) ? DX_TestPlugin_Helper::esc_attr( DX_TestPlugin_Helper::esc_sprint( $col_data['decoration']['col-sign-alt'] ) )  : ''
						);
					}

				} else {

					foreach ( (array)$test_plugin['signs'][$col_data['decoration']['col-sign-type']] as $sign ) {					
						if ( !empty( $sign['group_name'] ) && !empty( $sign['group_data'] ) ) {
							foreach ( (array)$sign['group_data'] as $sign_item ) {
								if ( isset( $sign_item['value'] ) && $sign_item['value'] == $col_data['decoration']['col-sign'][$col_data['decoration']['col-sign-type']] ) { 
									$sign_inner_html = sprintf( '<img src="%1$s" alt="%2$s">', 
										DX_TestPlugin_Helper::esc_attr( $sign_item['data'] ),
										isset( $col_data['decoration']['col-sign-alt'] ) ? DX_TestPlugin_Helper::esc_attr( DX_TestPlugin_Helper::esc_sprint( $col_data['decoration']['col-sign-alt'] ) ) : ''								
									);
								}
							}
							...
							if ( isset( $sign['value'] ) && $sign['value'] == $col_data['decoration']['col-sign'][$col_data['decoration']['col-sign-type']] ) {
								$sign_inner_html = sprintf( '<img src="%1$s" alt="%2$s">', 
									DX_TestPlugin_Helper::esc_attr( $sign['data'] ) ,
									isset( $col_data['decoration']['col-sign-alt'] ) ? DX_TestPlugin_Helper::esc_attr( DX_TestPlugin_Helper::esc_sprint( $col_data['decoration']['col-sign-alt'] ) ) : ''									
								);
							}
						}
				...
				if ( !empty( $sign_inner_html ) && !empty( $col_data['decoration']['col-sign-link']['url'] ) ) {
					...
					$sign_link_atts = apply_filters( 'test_plugin_ribbion_link_button_atts', 
						$sign_link_atts, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index
					...
					foreach ( (array)$sign_link_atts as $sign_link_att ) {
						if ( count( (array)$sign_link_att ) == 2 ) $sign_link_attributes[] = sprintf( '%1$s="%2$s"', $sign_link_att[0], DX_TestPlugin_Helper::esc_attr( $sign_link_att[1] ) );
					}					
					
					$sign_inner_html = sprintf(
						'<a href="%1$s"%2$s>%3$s</a>', 
						DX_TestPlugin_Helper::esc_attr( $col_data['decoration']['col-sign-link']['url'] ),
				...
				// Filter classes & styles
				$sign_classes = apply_filters( 'test_plugin_front_sign_classes', 
					$sign_classes, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index
				...
				$sign_styles = apply_filters( 'test_plugin_front_sign_styles', 
					$sign_styles, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index
				...
				$sign_html = sprintf( 
					'<div%1$s%2$s>%3$s</div>', 
					!empty( $sign_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$sign_classes  )  ) .  '"' : '',
					!empty( $sign_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$sign_styles )  ) .  ';"' : '',
					!empty( $sign_inner_html ) ? $sign_inner_html : ''					
				);
				
				// Filter html
				$sign_html = apply_filters( 'test_plugin_front_sign_html',
					$sign_html, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
			...
			if ( !empty( $column_style_data['type'] ) ) {
				...
				$header_classes = apply_filters( 'test_plugin_front_header_classes', 
					$header_classes, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				$header_styles = apply_filters( 'test_plugin_front_header_styles', 
					$header_styles, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				$price_format = apply_filters( 'test_plugin_front_price_format', 
					$price_format, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				switch( $column_style_data['type'] ) {	
					...
					case 'cpricing' :
						...
						switch( $price_type ) {
							...
							case 'price' :
								...
								$currency_symbol = '';
								foreach ( (array)$test_plugin['currency'] as $currency ) {
								...
								if ( isset( $col_data['price']['price'][0]['amount'][0] ) && $col_data['price']['price'][0]['amount'][0] != '' ) {
									...
									$price = sprintf( $price_format, 
										isset( $general_settings['currency'][0] ) ? DX_TestPlugin_Helper::esc_attr( json_encode( $general_settings['currency'][0] ) ) : '',
										$currency_symbol,
										number_format( 
					...
					case 'cpricing2' :
						...
						$header_custom_html = sprintf( 
							...
							!empty( $header_custom_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$header_custom_styles )  ) .  ';"' : ''
						);						
						break;									
						...
						$header_custom_html = sprintf( 
							...
							!empty( $col_data['header']['general']['custom']['img']['data'] ) ? 
								sprintf( 
									...
									isset( $col_data['header']['general']['custom']['img']['alt'] ) ? ' alt="' . DX_TestPlugin_Helper::esc_attr( $col_data['header']['general']['custom']['img']['alt'] ) . '"' : '',
			...
			$header_html = sprintf( 
				...
				!empty( $header_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$header_classes  )  ) .  '"' : '',
				!empty( $header_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$header_styles )  ) .  ';"' : '',
				$header_custom_html
			);
				
			// Filter html 
			$header_html = apply_filters( 'test_plugin_front_header_html',
				$header_html, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$header_data = apply_filters( 
				"test_plugin_front_header_html_{$pricing_table['style']}", 
				$header_html, 
				array( 
			...
			foreach ( (array)$pricing_table['col-data'][$col_index]['body-row'] as $body_row_index => $body_row_data ) {
				...
				if ( is_string( $body_row_data ) ) $body_row_data = DX_TestPlugin_Helper::parse_data( $body_row_data );
				...
				// Filter classes & styles
				$tooltip_classes = apply_filters( 'test_plugin_front_row_tooltip_classes', 
					$tooltip_classes, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				$tooltip_styles = apply_filters( 'test_plugin_front_row_tooltip_styles', 
					$tooltip_styles, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				$tooltip_html = isset( $body_row_data['type'] ) && isset( $body_row_data[$body_row_data['type']]['tooltip']['content'] ) && $body_row_data[$body_row_data['type']]['tooltip']['content'] != '' ?
					sprintf( 
						'<div%1$s%2$s>%3$s</div>', 
						!empty( $tooltip_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$tooltip_classes  )  ) .  '"' : '',
						!empty( $tooltip_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$tooltip_styles )  ) .  ';"' : '',
						$body_row_data[$body_row_data['type']]['tooltip']['content']
					) : '';
				
				// Filter html 
				$tooltip_html = apply_filters( 'test_plugin_front_row_tooltip_html',
					$tooltip_html, 
					array(
						'pricing_table' => $pricing_table,
						'col_index' => $col_index,
				...
				if ( !empty( $body_row_data['type'] ) ) {
					...
					// Filter classes & styles
					$body_row_classes = apply_filters( 'test_plugin_front_row_classes', 
						$body_row_classes, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
					...
					$body_row_styles = apply_filters( 'test_plugin_front_row_styles', 
						$body_row_styles, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
					...
					$body_row_inline_styles = apply_filters( 
						"test_plugin_front_body_row_styles_{$pricing_table['style']}",
						array(),
						array( 
							'col_index' => $col_index,
							'row_index' => $body_row_index,
					...
					switch( $body_row_data['type'] ) {
						...
						case 'button' : 
							...
							// Filter classes & atts
							$button_classes = apply_filters( "test_plugin_front_body_button_classes_{$pricing_table['style']}", 
								$button_classes, 
								array( 
									'col_index' => $col_index,
									'row_index' => $body_row_index,
							...
							$button_atts = apply_filters( 'test_plugin_front_body_button_atts', 
								$button_atts, 
								array(
									'pricing_table' => $pricing_table,
									'col_index' => $col_index,
							...
							foreach ( (array)$button_atts as $button_att ) {
								if ( count( (array)$button_att ) == 2 ) $button_attributes[] = sprintf( '%1$s="%2$s"', $button_att[0], DX_TestPlugin_Helper::esc_attr( $button_att[1] ) );
							}
									...
									case 'button' :
										$button_html = isset( $body_row_data[$body_row_data['type']]['code'] ) && $body_row_data[$body_row_data['type']]['code'] != '' && isset( $body_row_data[$body_row_data['type']]['content'] ) && $body_row_data[$body_row_data['type']]['content'] != ''  ? 
											sprintf(
												...
												DX_TestPlugin_Helper::esc_attr( $body_row_data[$body_row_data['type']]['code'] ),
												!empty( $button_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$button_classes )  ) .  '"' : '',
							...
							// Filter footer html
							$button_html = apply_filters( 'test_plugin_front_body_button_html', 
								$button_html, 
								array(
									'pricing_table' => $pricing_table,
									'col_index' => $col_index,
					...
					$body_row_html = isset( $body_row_data ) ? 
						sprintf(
							...
							!empty( $body_row_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$body_row_classes  )  ) .  '"' : '',
							!empty( $body_row_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$body_row_styles )  ) .  ';"' : '',
							$body_row_inner_content
						) : '';										
					
					// Filter html 
					$body_row_html = apply_filters( 'test_plugin_front_body_row_html',
						$body_row_html, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
			...
			// Filter classes & styles
			$body_classes = apply_filters( 'test_plugin_front_body_classes', 
				$body_classes, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$body_styles = apply_filters( 'test_plugin_front_body_styles', 
				$body_styles, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$body_html = !empty( $body_rows_html ) ? 
				sprintf(
					'<ul%1$s%2$s>%3$s</ul>', 
					!empty( $body_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$body_classes  )  ) .  '"' : '',
					!empty( $body_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$body_styles )  ) .  ';"' : '',
					$body_rows_html
				) : '';		 
			
			
			// Filter html
			$body_html = apply_filters( 'test_plugin_front_body_html',
				$body_html, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			foreach ( (array)$pricing_table['col-data'][$col_index]['footer-row'] as $footer_row_index => $footer_row_data ) {
				...
				if ( is_string( $footer_row_data ) ) $footer_row_data = DX_TestPlugin_Helper::parse_data( $footer_row_data );
				...
				if ( !empty( $footer_row_data['type'] ) ) {
					...
					// Filter classes & styles
					$footer_row_classes = apply_filters( 'test_plugin_front_row_classes', 
						$footer_row_classes, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
					...
					$footer_row_styles = apply_filters( 'test_plugin_front_row_styles', 
						$footer_row_styles, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
					...
					$footer_row_inline_styles = apply_filters( 
						"test_plugin_front_footer_row_styles_{$pricing_table['style']}",
						array(),
						array( 
							'col_index' => $col_index,
							'row_index' => $footer_row_index,
					...
					switch( $footer_row_data['type'] ) {
						...
						case 'button' : 
							...
							// Filter classes & atts
							$button_classes = apply_filters( "test_plugin_front_footer_button_classes_{$pricing_table['style']}", 
								$button_classes, 
								array( 
									'col_index' => $col_index,
									'row_index' => $footer_row_index,
							...
							$button_atts = apply_filters( 'test_plugin_front_footer_button_atts', 
								$button_atts, 
								array(
									'pricing_table' => $pricing_table,
									'col_index' => $col_index,
								...
								switch( $button_data['type'] ) {
									case 'button' :
										$button_html = isset( $button_data['code'] ) && $button_data['code'] != '' && isset( $button_data['content'] ) && $button_data['content'] != ''  ? 
											sprintf(
												'<a href="%1$s"%2$s%3$s><span class="gw-go-btn-inner">%4$s</span></a>', 
												DX_TestPlugin_Helper::esc_attr( $button_data['code'] ),
												!empty( $button_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$button_classes )  ) .  '"' : '',
												!empty( $button_attributes ) ? ' ' . implode( ' ', (array)$button_attributes ) : '',
												$button_data['content']
											) : '';
							...
							// Filter footer html
							$button_html = apply_filters( 'test_plugin_front_footer_button_html', 
								$button_html, 
								array(
									'pricing_table' => $pricing_table,
									'col_index' => $col_index,
					...
					$footer_row_html = !empty( $footer_row_data ) ? 
						sprintf(
							...
							!empty( $footer_row_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$footer_row_classes  )  ) .  '"' : '',
							!empty( $footer_row_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$footer_row_styles )  ) .  ';"' : '',
							$footer_row_inner_content
						) : '';						
					
					// Filter html 
					$footer_row_html = apply_filters( 'test_plugin_front_footer_row_html',
						$footer_row_html, 
						array(
							'pricing_table' => $pricing_table,
							'col_index' => $col_index,
			...
			// Filter classes & styles
			$footer_classes = apply_filters( 'test_plugin_front_footer_classes', 
				$footer_classes, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$footer_styles = apply_filters( 'test_plugin_front_footer_styles', 
				$footer_styles, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
			...
			$footer_html = !empty( $footer_rows_html ) ? 
				sprintf(
					...
					!empty( $footer_classes ) ? ' class="' . DX_TestPlugin_Helper::esc_attr( implode( ' ', (array)$footer_classes  )  ) .  '"' : '',
					!empty( $footer_styles ) ? ' style="' . DX_TestPlugin_Helper::esc_attr( implode( '; ', (array)$footer_styles )  ) .  ';"' : '',
					$footer_rows_html
				) : '';		 
			
			
			// Filter html
			$footer_html = apply_filters( 'test_plugin_front_footer_html',
				$footer_html, 
				array(
					'pricing_table' => $pricing_table,
					'col_index' => $col_index,
		...
		$html = apply_filters( 'test_plugin_front_html', $html, $pricing_table );

		$inline_css = '';
		$font_css = '';
		$test_plugin['_frontend']['fonts'] = array();
		
		foreach( (array)$google_fonts as $font ) {
			if ( !in_array( $font, (array)$test_plugin['_frontend']['fonts'] ) ) {			
				$font_css .= ' ' . $font . ';';
				$test_plugin['_frontend']['fonts'][] = $font;
			}
		}
			
		$custom_inline_styles[] = '.gw-go { visibility:inherit; }';
		
		$custom_inline_styles = apply_filters ( 'test_plugin_css', $custom_inline_styles );
				...
				foreach ( $pricing_table['responsivity']['views'] as $view ) {
					...
					if ( !empty( $mq ) ) {						
						$mq_css .= sprintf( 
							...
							implode ( ' and ', $mq),
							sprintf( '#test-plugin-table-%1$s .gw-go-col-wrap { width:%2$d%%; }', $postid, $width )
						);
					}
		...
		if ( !empty( $inline_css ) ) {
			...
			/* Remove styles with "#test-plugin-table-xxx" id prefix from custom CSS */
			$custom_css = preg_replace( '/[^\{\}]+#test-plugin-table-[0-9]+[^\{\}]+{[^\}]+}/', '', $custom_css );
			$custom_css = apply_filters( 'test_plugin_front_custom_css', $custom_css, $pricing_table );
			...
			if ( !empty( $custom_css_mqs ) ) {
				...
				foreach( $custom_css_mqs as $custom_css_mq ) {
					...
					if ( !empty( $custom_css_mq[2] ) )  {
						...
						$custom_css_mq[2] = preg_replace( '/([a-z\.#\[\]][a-z0-9\-\_\[\]\"\=\:\(\)\*\$\^\|\~\+\>\*\.\s#]+|\*)(?=\s?,(?=[^}]*{)|\s*{)/', "" .' #test-plugin-table-' . $postid . ' $0', $custom_css_mq[2] );
			...
			$inline_css = preg_replace( '/([a-z\.#\[\]][a-z0-9\-\_\[\]\"\=\:\(\)\*\$\^\|\~\+\>\*\.\s#]+|\*)(?=\s?,(?=[^}]*{)|\s*{)/', "" .' #test-plugin-table-' . $postid . ' $0', $inline_css );
	...
	/**
	 * HTML5 video shortcode [test_plugin_html5_video]
	 *
	 * @return string Returns HTML code.
	 */	
...
/*		return '<video controls="controls"' . ( $autoplay != '' ? $autoplay : '' ) . ( $loop != '' ? $loop : '' ) . ( $poster_src != '' ? ' poster="' . $poster_src . '"' : '' ) .'>' . $mp4_src . $webm_src . $ogg_src . '<object type="application/x-shockwave-flash" data="' . $this->plugin_url . 'assets/plugins/js/mediaelementjs/flashmediaelement.swf">
	...
	/**
	 * Youtube video shortcode [test_plugin_youtube]
	 *
	 * @return string Returns HTML code.
	 */		
	...
	/**
	 * Vimeo video shortcode [test_plugin_vimeo]
	 *
	 * @return string Returns HTML code.
	 */
	...
	/**
	 * Screenr video shortcode [test_plugin_screenr]
	 *
	 * @return string Returns HTML code.
	 */
	...
	/**
	 * Dailymotion video shortcode [test_plugin_dailymotion]
	 *
	 * @return string Returns HTML code.
	 */	
	...
	/**
	 * Metacafe video shortcode [test_plugin_metacafe]
	 *
	 * @return string Returns HTML code.
	 */	
	...
	/**
	 * HTML5 audio shortcode [test_plugin_audio]
	 *
	 * @return string Returns HTML code.
	 */
	...
	/**
	 * Soundcloud audio shortcode [test_plugin_soundcloud]
	 *
	 * @return string Returns HTML code.
	 */	
	...
	/**
	 * Mixcloud audio shortcode [test_plugin_mixcloud]
	 *
	 * @return string Returns HTML code.
	 */	
	...
	/**
	 * Beatport audio shortcode [test_plugin_beatport]
	 *
	 * @return string Returns HTML code.
	 */		
	...
	/**
	 * Google map shortcode [test_plugin_map]
	 *
	 * @return string Returns HTML code.
	 */	
	...
	/**
	 * IFRAME shortcode [test_plugin_custom_iframe]
	 *
	 * @return string Returns HTML code.
	 */	
