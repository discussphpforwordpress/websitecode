			...
			tb_position = function() {
				if ( ! _self._vars.tb_icon_bar_open ) {
					...
					if ( ! _self._vars.tb_unload_binded ) {
						...
						$tbWindow.append('<div class="submit-wrap"><div class="optionsframework-submit"><a href="#closeTB" class="button-primary">更改并关闭</a><div class="clear"></div></div></div>');
...
jQuery(function($){
	$('.section-background_img .of-radio-img-img').on('click', function() {
		...
		if ( screenshot.find('img').length > 0 ) {
			...
			screenshot.empty().append('<img src="' + attachment + '"><a class="remove-image">删除</a>').slideDown('fast');
		}
