...
if ( ! function_exists( 'presscore_get_the_mobile_logo' ) ) :
	...
	function presscore_get_the_mobile_logo() {
		...
		if ( 'mobile' === $config->get( 'header.mobile.logo.first_switch' ) || 'mobile' === $config->get( 'header.mobile.logo.second_switch' ) ) {
			$logo .= presscore_get_logo_image( array(
		...
		if ( 'none' !== $transparent_logo_style && 'desktop' === $config->get( 'header.mobile.logo.second_switch' ) && presscore_header_is_transparent() ) {
			$logo .= presscore_get_logo_image( array(
				'logo' 			=> dt_get_uploaded_logo( $config->get( 'logo.header.regular' ) ),
				'logo_retina'	=> dt_get_uploaded_logo( $hd_logo = $config->get( 'logo.header.hd' ), 'retina' ),
			), 'mobile-desktop-logo' );
		}
